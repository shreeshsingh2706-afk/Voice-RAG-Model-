...
def voice_rag_pipeline(
    ...
    PARAMETERS:
        ...
        language_code : "en-IN", "hi-IN", or "unknown"
    ...
    if stt_result["status"] == "error":
        return {
            "transcript": "",
            "answer":     f"Speech-to-text failed: {stt_result.get('error', 'Unknown error')}",
            "confidence": "none",
            "status":     "stt_error",
            "sources":    [],
            "latency":    {"stt_ms": stt_result.get("stt_ms", 0), "total_ms": 0},
    ...
    if not transcript:
        return {
            "transcript": "",
            "answer":     "Could not detect any speech in the audio. Please speak clearly and try again.",
            "confidence": "none",
            "status":     "empty_transcript",
            "sources":    [],
            "latency":    {"stt_ms": stt_result.get("stt_ms", 0), "total_ms": 0},
    ...
    combined_latency = {
        "stt_ms":    stt_result.get("stt_ms", 0),
        "vector_ms": rag_latency.get("vector_ms", 0),
        "bm25_ms":   rag_latency.get("bm25_ms", 0),
        "rerank_ms": rag_latency.get("rerank_ms", 0),
        "llm_ms":    rag_latency.get("llm_ms", 0),
        "total_ms":  total_ms,
    }
